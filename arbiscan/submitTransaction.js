...
const sign = async function (ledger, tx, nonce) {
  ...
  switch (args[0]) {
    case 'USDT':
      token = 'Fd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9'
      break
    case 'EURT':
      token = ''
      break
    case 'XAUT':
      ...
      break
    case 'ADMIN':
      token = '4a9196b06f339Ad9F3Ee752C987b401F2E1E2718'
      break
  }
  ...
  switch (args[1]) {
    case 'issue':
      ...
      console.log('Note: arbitrium uses "mint dst-address amount"')
      return
      break
    case 'redeem':
  ...
  console.log('\nRequesting Ledger Sign: GasPrice: \x1b[32m%s\x1b[0m Gwei, Nonce: \x1b[32m%s\x1b[0m, TX: \x1b[32m%s\x1b[0m, InputData: \x1b[32m%s\x1b[0m',gasPrice/10e8,parseInt(nonce),tx,data,)
  let result
  try {
/*
      let loadConfig={
...
async function updateGas () {
    ethGasStationData = await request
      .get({
        url: 'https://api.arbiscan.io/api?module=proxy&action=eth_gasPrice&apikey='+apikey,
        json: true
      })
...
async function broadcastSnowtrace (signedtx) {
    ...
    var options = {
      url: "https://api.arbiscan.io/api?module=proxy&action=eth_sendRawTransaction&hex="+signedtx+"&apikey="+apikey,
      json: true
    }
